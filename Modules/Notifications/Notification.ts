...
export default class Notification {
    ...
    constructor(
        htmlOrHTMLElement: string | HTMLElement,
        type: string,
        userOptions: FLib.Notifications.Options,
        close: (notification: Notification) => void,
        notificationOptions?: FLib.Notifications.NotificationOptions,
    ) {
        ...
        if (!notificationOptions) {
            ...
            notificationOptions &&
            notificationOptions.closeDelay !== false
        ) {
            this.#closeDelay =
                notificationOptions.closeDelay || this.#options.autoCloseDelay;
        }
    ...
    show($list: HTMLElement): Promise<any> {
        if (!this.#$notification) {
            this.#$notification = strToDOM(
                this.#options.templates.notification,
            ) as HTMLElement;
            (this.#$notification as HTMLElement).replaceChildren(
                this.#html || "",
            );
        ...
        return wait(100).then(() =>
            this.#options.animations.show(this.#$notification, this.#options),
        );
    }
    ...
    hide(): Promise<any> {
        ...
        return this.#options.animations
            .hide(this.#$notification, this.#options)
            .then(() => {
