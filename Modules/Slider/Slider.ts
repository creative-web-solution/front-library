...
export default class Slider {
    ...
    #lastMoveData?: FLib.Slider.CallbackParam;
    ...
    get lastMoveData(): FLib.Slider.CallbackParam | undefined {
        return this.#lastMoveData;
    }

    constructor(
        $slider: HTMLElement,
        userOptions: Partial<FLib.Slider.Options> = {},
    ) {
        ...
        this.#$slider = $slider;
        this.#$list = $slider.querySelector(
            this.#options.listSelector,
        ) as HTMLElement;
        this.#$slides = this.#$list.querySelectorAll(
        ...
        if (!this.#$list.id) {
            this.#$list.id = this.#createListId();
        }
        ...
        for (let index = 0, len = this.#$slides.length; index < len; ++index) {
            ...
            const slide = new Slide({
                ...
                index,
                slideAriaLabel: this.#options.slideAriaLabel,
    ...
    #createListId(): string {
        return `__mdl_lst_-${customeListId++}`;
    }
    ...
    #getNextSlideIndex = (index: number, step = 1): number => {
        ...
        if (this.#options.loop && index >= this.#nbSlides) {
            ...
            (!this.#options.loop &&
                !this.#options.moveByPage &&
                index > this.#nbSlides - this.#options.slidePerPage) ||
            index >= this.#nbSlides
        ) {
            return -1;
        }
    ...
    #reorderSlidesWithLoop = (
        activeSlide: Slide,
        direction: FLib.Slider.SlideDirection,
    ): void => {
        ...
        if (direction === DIRECTION_NEXT) {
            nbSlideAfter = Math.floor(
                (this.#nbSlides - this.#options.slidePerPage) / 2,
            );
            nbSlideBefore =
                this.#nbSlides - this.#options.slidePerPage - nbSlideAfter;
        } else {
            nbSlideBefore = Math.floor(
                (this.#nbSlides - this.#options.slidePerPage) / 2,
            );
            nbSlideAfter =
                this.#nbSlides - this.#options.slidePerPage - nbSlideBefore;
        }
    ...
    #reorderSlidesWithoutLoop = (activeSlide: Slide): void => {
        for (let i = 0; i < this.#nbSlides; ++i) {
            ...
            if (i === activeSlide.index) {
                ...
                i <
                activeSlide.index - this.#options.nbSlideVisibleBefore
            ) {
    ...
    #getShortestWayAndDirection = (
        index: number,
        askedDirection?: FLib.Slider.SlideDirection,
    ): Record<string, any> => {
        ...
        if (askedDirection === DIRECTION_PREVIOUS) {
            ...
            setupOpt.nbStepsMin = setupOpt.straightMove
                ? setupOpt.nbStepsStraight
                : setupOpt.nbStepsViaStart;
    ...
    #moveTo = (
        ...
        const wayAndDirection = this.#getShortestWayAndDirection(
            index,
            askedDirection,
        );
        const fun =
        ...
        this.#lastMoveData = callbackData;
    ...
    next($button?: HTMLElement): Promise<void> {
        if (
            this.#state !== this.#STATE_IDLE ||
            (!this.#options.loop && this.#currentSlide.isLast)
        ) {
            return Promise.resolve();
        }
    ...
    previous($button?: HTMLElement): Promise<void> {
        if (
            this.#state !== this.#STATE_IDLE ||
            (!this.#options.loop && this.#currentSlide.isFirst)
        ) {
            return Promise.resolve();
        }
    ...
    goTo(page: number, $button?: HTMLElement): Promise<void> {
        const index = this.#options.moveByPage
            ? page * this.#options.slidePerPage
            : page;
