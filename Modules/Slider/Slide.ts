...
export class Slide {
    ...
    constructor(options: FLib.Slider.SlideOptions) {
        ...
        this.#$links = this.#$slide.querySelectorAll(
        ...
        this.#currentPage =
            Math.floor(options.index / options.slidePerPage) + 1;
        ...
        this.#$slide.setAttribute("role", "group");
        this.#$slide.setAttribute("aria-roledescription", "slide");
        this.#$slide.setAttribute(
            "aria-rlabel",
            options.slideAriaLabel
                .replace("%p", String(this.position))
                .replace("%t", String(options.nbSlides)),
        );
    }
    ...
    #getXMin = (offset: number): number => {
        return (
            this.#size *
            Math.max(offset, -1 * this.#options.nbSlideVisibleBefore - 1)
        );
    };
