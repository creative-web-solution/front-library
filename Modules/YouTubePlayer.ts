...
export default class YouTubePlayer {
    ...
    #loadYouTubeAPI = (): Promise<void> => {
        ...
        (window as FLib.YouTubePlayer.PlayerWindow).onYouTubeIframeAPIReady =
            function () {
                _resolve();

                (
                    window as FLib.YouTubePlayer.PlayerWindow
    ...
    #initPlayer = (
        $wrapper: FLib.YouTubePlayer.PlayerWrapper,
    ): Promise<YT.Player> => {
        let _resolve;
    ...
    load(): Promise<YT.Player> {
        return this.#loadYouTubeAPI().then(() => {
            return this.#initPlayer(
                this.#options.$wrapper as FLib.YouTubePlayer.PlayerWrapper,
            );
        });
    }
}
